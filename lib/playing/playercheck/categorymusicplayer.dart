...
class categoryplayer extends StatefulWidget {
  Categorymusicmodel music;
  List<Categorymusicmodel> allSong;
  String cid;
  String userid;
  categoryplayer(
      {Key? key,
...
class _PlayingWidgetState extends State<categoryplayer> {
  ...
  List<Categorymusicmodel> _playedSongs = [];
  ...
  _fetchMusicData() async {
    final response = await http.post(
        Uri.parse('https://admin.koinoniaconnect.org/API/categoryMusic'),
        body: {'user_id': widget.userid, 'category_id': widget.cid});
  ...
  Future<void> _addPlayedSong(Categorymusicmodel music) async {
    ...
    bool isAlreadyPlayed = playedSongsJson.any((songJson) {
      final song = Categorymusicmodel.fromJson(json.decode(songJson));
