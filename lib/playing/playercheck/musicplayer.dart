...
class PlayingWidget2 extends StatefulWidget {
  Musics singleMusic;
  List<Musics> allMusicList;
  String userid;
  PlayingWidget2(
      {Key? key,
...
class _PlayingWidgetState extends State<PlayingWidget2> {
  ...
  //     setState(() {
  //       _musicList =
  //       List<Musics>.from(data.map((json) => Musics.fromJson(json)));
  //     });
  //   } else {
  ...
  void _initAudioPlayer() async {
    await _audioPlayer.setUrl(
        "https://admin.koinoniaconnect.org/" + widget.singleMusic.musicFile);
  ...
  void checklike() async {
    ...
    var response = await http.post(url, body: {
      ...
      'music_id': widget.singleMusic.musicId,
    });
  ...
  Future<void> _addPlayedSong(Musics music) async {
    ...
    bool isAlreadyPlayed = playedSongsJson.any((songJson) {
      final song = Musics.fromJson(json.decode(songJson));
  ...
  void initState() {
    ...
    // _fetchMusicData();
    _musicList = widget.allMusicList;
    myCurrentImageIndex = _musicList.indexOf(widget.singleMusic);
    userid = "";
    super.initState();
    ...
    print("userid " + userid);
    print("music id " + widget.singleMusic.musicId);
  }
  ...
  void _play() {
    final _musicList = widget.singleMusic.copyWith(userId: userid);
  ...
  getnextsongimage() async {
    int nextIndex =
        (_musicList.indexOf(widget.singleMusic) + 1) % _musicList.length;
    Musics nextimage = _musicList[nextIndex];
    setState(() {
      widget.singleMusic = nextimage;
  ...
  _next() async {
    ...
    if (_repeathasBeenPressed) {
      ...
      // Load the next track
      Musics nextMusic = _musicList[nextIndex];
  ...
  void _previous() async {
    ...
    Musics previousMusic = _musicList[previousIndex];
    await _audioPlayer.setAudioSource(
      AudioSource.uri(Uri.parse(
    ...
    setState(() {
      widget.singleMusic = previousMusic;
  ...
  Future<void> onLikeButtonTapped(bool isLiked) async {
    if (!isLiked) {
      await networkutils.addtoliked(userid, widget.singleMusic.musicId);

      print("liked");
    } else {
      print("unliked");
      await networkutils.deletelikedmusic(userid, widget.singleMusic.musicId);
    }
        ...
        title: Column(
          ...
          children: [
            ...
            Align(
              alignment: AlignmentDirectional(-0.2, 0),
              child: Text(
                widget.singleMusic.albumName ?? "not set",
                      ...
                      children: [
                        ...
                        Row(
                          ...
                          children: [
                            Expanded(
                              child: Text(
                                widget.singleMusic.artists!.isEmpty
                                    ? ""
                                    : widget.singleMusic.artists![0]
                                            .artistName ??
                                        "",
                                textAlign: TextAlign.center,
                                style: FlutterFlowTheme.of(context)
