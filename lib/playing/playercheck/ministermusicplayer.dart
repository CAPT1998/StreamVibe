...
class ministermusicplayer extends StatefulWidget {
  Ministermusicmodel music;
...
class _PlayingWidgetState extends State<ministermusicplayer> {
  ...
  List<Ministermusicmodel> _musicList = [];
  final AudioPlayer audioPlayer = AudioPlayer();
  late List<String> _imageUrls;
  List<Ministermusicmodel> _playedSongs = [];
  ...
  _fetchMusicData() async {
    final response = await http.post(
        Uri.parse('https://admin.koinoniaconnect.org/API/getArtistMusic'),
        body: {'user_id': widget.userid, 'artist_id': widget.mid});
    if (response.statusCode == 200) {
      final data = jsonDecode(response.body);
      setState(() {
        _musicList = List<Ministermusicmodel>.from(
  ...
  Future<void> _addPlayedSong(Ministermusicmodel music) async {
    print("music image = ${music}");
    ...
    bool isAlreadyPlayed = playedSongsJson.any((songJson) {
      final song = Ministermusicmodel.fromJson(json.decode(songJson));
  ...
  getnextsongimage() async {
    ...
    Ministermusicmodel nextimage = _musicList[nextIndex];
    setState(() {
  ...
  _next() async {
    ...
    if (_repeathasBeenPressed) {
      ...
      // Load the next track
      Ministermusicmodel nextMusic = _musicList[nextIndex];
  ...
  void _previous() async {
    ...
    Ministermusicmodel previousMusic = _musicList[previousIndex];
