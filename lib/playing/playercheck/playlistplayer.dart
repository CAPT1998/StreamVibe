...
class playlistplayer extends StatefulWidget {
  Platylistmusic music;
  List<Platylistmusic> allSongs;
  String pid;
  String userid;
  playlistplayer(
      {Key? key,
...
class _PlayingWidgetState extends State<playlistplayer> {
  ...
  List<Platylistmusic> _musicList = [];
  final AudioPlayer audioPlayer = AudioPlayer();
  late List<String> _imageUrls;
  List<Platylistmusic> _playedSongs = [];
  ...
  //       .post(Uri.parse('https://admin.koinoniaconnect.org/API/getPlaylistMusic'),
  //       body: {
  //         'user_id': widget.userid,
  //     'user_playlist_id': widget.pid
  //       });
  //   if (response.statusCode == 200) {
  //     final data = jsonDecode(response.body);
  //     setState(() {
  //       _musicList =
  //           List<Platylistmusic>.from(data.map((json) => Platylistmusic.fromJson(json)));
  //       myCurrentImageIndex = _musicList.indexOf(widget.music);
  //       print("_musicList: $_musicList");
  //       print("widget.music: ${widget.music}");
  //
  ...
  Future<void> _addPlayedSong(Platylistmusic music) async {
    ...
    bool isAlreadyPlayed = playedSongsJson.any((songJson) {
      final song = Platylistmusic.fromJson(json.decode(songJson));
  ...
  getnextsongimage() async {
    ...
    Platylistmusic nextimage = _musicList[nextIndex];
    setState(() {
  ...
  _next() async {
    ...
    if (_repeathasBeenPressed) {
      ...
      // Load the next track
      Platylistmusic nextMusic = _musicList[nextIndex];
  ...
  void _previous() async {
    ...
    Platylistmusic previousMusic = _musicList[previousIndex];
    await _audioPlayer.setAudioSource(
      AudioSource.uri(Uri.parse(
